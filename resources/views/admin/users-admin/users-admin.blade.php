...
@section('content')
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex gap-x-3">
            <x-ui.button-export  href="{{ route('admin.users.export.admin') }}" />
            <x-ui.button-add href="{{ route('admin.users.create.admin') }}">
                Users
            </x-ui.button-add>
        </div>
    </div>
                    ...
                    @foreach ($admins as $index => $value)    
                        <tr class="text-center">
                            ...
                            <td class=" py-3 px-2 flex gap-x-3 items-center justify-center">
                                <x-ui.button-edit href="{{ route('admin.users.admin.edit', $value->id) }}" />
                                <x-ui.button-delete href="{{ route('admin.users.destroy.admin', $value->id) }}" />
                            </td>
                        </tr>
                    @endforeach
                </tbody>
