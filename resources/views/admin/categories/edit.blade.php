...
@section('content')
    ...
    <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" class="w-1/2 mt-5 flex flex-col gap-y-3" >
        @csrf
        @method('PATCH')
        ...
        <div class="flex flex-col gap-y-3" id="container">
            <div class="form-item flex flex-col gap-y-3">
                ...
                <select name="division" class="w-full px-2 py-2 border-[#575757] border rounded-md text-sm focus:border-[#575757] outline:border-[#575757] text-white/70">
                    ...
                    <option class="bg-[#222]" value="sarpras" {{ $category->division == 'sarpras' ? 'selected' : '' }}>Sarpras</option>
                    <option class="bg-[#222]" value="tefa" {{ $category->division == 'tefa' ? 'selected' : '' }}>Tefa</option>
                    <option class="bg-[#222]" value="tata_usaha" {{ $category->division == 'tata_usaha' ? 'selected' : '' }}>Tata Usaha</option>
                </select>
                 @error('division')
                    <x-ui.message>{{ $message }}</x-ui.message>
                @enderror
